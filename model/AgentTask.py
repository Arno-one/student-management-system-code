...
class AgentTask(Base):
    ...
    intent = Column(String(50), index=True, comment="识别到的意图")
    ...
    status = Column(String(30), index=True, default="pending", comment="状态: pending/running/awaiting_hitl/success/partial_success/clarification/empty/error/cancelled")
    total_duration_ms = Column(Integer, default=0, comment="总耗时（毫秒）")
    create_time = Column(DateTime, index=True, default=datetime.now, comment="创建时间")
