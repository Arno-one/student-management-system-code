...
class IntentResult(BaseModel):
    intent: str = Field(description="意图类型: score_query / academic_advice / knowledge_qa / data_query / weather_query / email_draft / emotional_support / general_chat")
...
6. 如果用户请求写邮件、发邮件、回复邮件 → email_draft
7. 如果用户表达情绪、压力、烦恼且不带明确查询需求 → emotional_support
8. 以上都不匹配 → general_chat
