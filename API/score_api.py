...
from scheme.schema_score import Addscore,Updatescore,List
...
def add_score_api(new_score:Addscore,db=Depends(get_db)):
    ...
    return {"message": "学生成绩信息添加成功",
            "添加的数据为": {"student_no": result.student_no,"exam_order": result.exam_order,"score": result.score}}

#批量添加
@router_score.post("/batch/add", summary="批量添加学生成绩")
...
#查询学生成绩接口
@router_score.get("/query/one",summary="查询学生的成绩")
def query_score_api(db=Depends(get_db),
    ...
    result = score_dao.query_score(db,page,page_size,student_no,exam_order,min_score,max_score,sort_no,sort_score)
